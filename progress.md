...
- [x] `docker compose up -d --build` 전체 서비스 기동 확인
- [x] API / UI / worker 기본 헬스체크 통과
...
- [x] CI/CD 파이프라인 — PR #36
- [x] Dockerfile multi-stage 프로덕션 빌드 — PR #36
...
- [x] event loop 오염 해결 — conftest.py event_loop fixture 제거 + IsolatedAsyncioTestCase 전환 (PR #50/#55)
- [x] DB 미연결 3건 — `@pytest.mark.integration` 마킹 (PR #50/#55)
- [x] 인터페이스 불일치 71건 추가 수정 + Settings 캐시 autouse clear (PR #55)
- **최종: 557 passed, 0 failed**
...
- ~~테스트 event loop 오염~~ — **해결 (PR #50/#55)**. conftest.py event_loop fixture 제거 + IsolatedAsyncioTestCase 전환
