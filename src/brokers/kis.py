"""
src/brokers/kis.py — KIS 모의투자 브로커 및 API 클라이언트
"""
...
from src.services.kis_session import get_stored_kis_token
...
class KISPaperApiClient:
    def __init__(
        self,
        *,
        settings: Settings | None = None,
        token_provider=get_stored_kis_token,
    ) -> None:
        self.settings = settings or get_settings()
        self._token_provider = token_provider

    def is_configured(self) -> bool:
        return bool(self.settings.kis_app_key and self.settings.kis_app_secret and self.settings.kis_account_number)

    def _account_parts(self) -> tuple[str, str]:
        digits = "".join(ch for ch in self.settings.kis_account_number if ch.isdigit())
        if len(digits) < 10:
    ...
    async def _base_headers(self, tr_id: str) -> dict[str, str]:
        ...
        if not token:
            raise KISAPIError("Redis에 저장된 KIS 토큰이 없습니다. `python scripts/kis_auth.py`를 먼저 실행하세요.")
    ...
    async def _create_hashkey(self, payload: dict, tr_id: str) -> str:
        ...
        url = f"{self.settings.kis_base_url_for_scope('paper')}/uapi/hashkey"
    ...
    async def _request_json(
        ...
        *,
        ...
        url = f"{self.settings.kis_base_url_for_scope('paper')}{path}"
        async with httpx.AsyncClient(timeout=self.settings.kis_request_timeout_seconds) as client:
    ...
    async def place_order(self, order: PaperOrderRequest) -> KISOrderReceipt:
        if not self.is_configured():
            raise KISAPIError("KIS 설정이 비어 있어 모의 주문 API를 사용할 수 없습니다.")

        cano, account_product = self._account_parts()
        tr_id = "VTTC0802U" if order.signal == "BUY" else "VTTC0801U"
        ord_dvsn = "01"
        payload = {
            "CANO": cano,
...
class KISPaperBroker:
    def __init__(
        ...
        *,
        ...
        self.fallback_broker = fallback_broker or PaperBroker()
        self.client = client or KISPaperApiClient(settings=self.settings)
    ...
    async def execute_order(self, order: PaperOrderRequest) -> PaperBrokerExecution:
        ...
        if self.execution_mode == "kis_shadow":
